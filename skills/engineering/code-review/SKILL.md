...
diff 引入的类型、枚举成员、事件、消息、命令、队列项这类跨过函数或模块边界的值，逐个去读消费端的 dispatch 点（switch、router、过滤器、handler 注册表、循环体），确认它有显式分支或已有的 catch-all 正确接收；静默丢弃、无操作、`return` 了事都算发现。dispatch 点常常在 diff 之外——读完消费端之前，不要断定生产端没问题。
