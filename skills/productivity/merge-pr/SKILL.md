...
gh api --method PUT repos/<owner>/<repo>/pulls/<层PR号>/merge-async \
  -f merge_method=<squash|merge|rebase> \
  -f commit_title="<type>(<scope>): <描述> (#<层PR号>)" \
  -f commit_message="<改动摘要>"
```

- `squash` 必须显式给 `commit_title`（`merge` / `rebase` 不给，它们沿用分支上的提交信息）：`commit_title` 只作用于本次请求的目标层。不给时服务端按仓库的 `squash_merge_commit_title` 生成，多提交的层会取 PR 标题，AI 标记就进了提交历史。标题按该层分支的 commit 约定写（缺省约定见 `git-commit` 的 `references/git-workflow.md`），正文写改动摘要。
- 响应里取 `details.uuid` 轮询：
